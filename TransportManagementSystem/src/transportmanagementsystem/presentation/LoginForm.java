    ...
    private void loginUser() {
        ...
        try (Connection con = DatabaseConnection.getConnection();
            ...
            if (rs.next()) {
                ...
                failedAttempts = 0;
                 UserLogin user = new UserLogin(email, password); // ✅ Create user
            UserLogin.setCurrentUser(user);  
            String name = rs.getString("name"); // Assuming your users table has a "name" column

UserLogin.setCurrentUser(user);
UserLogin.setLoggedInUsername(name);  // ✅ Store name for global access
